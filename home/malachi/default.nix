...
# ---------------------------------------------------------------------------
{
  ...
  firefoxExistingProfile = pkgs.writeShellScriptBin "firefox-existing-profile" ''
    set -euo pipefail
    ...
    if [ -r "$profiles_ini" ]; then
      rel_path="$(${pkgs.gawk}/bin/awk -F= '
        ...
        in_profile && default_profile && path != "" {
          print path
          exit
        }
      ' "$profiles_ini")"
      ...
      if [ -n "$rel_path" ]; then
        case "$rel_path" in
          ...
          *) profile_path="$profile_root/$rel_path" ;;
        esac
      fi
    fi

    if [ -n "$profile_path" ] && [ -d "$profile_path" ]; then
      exec ${pkgs.firefox}/bin/firefox --name firefox --profile "$profile_path" "$@"
    fi

    exec ${pkgs.firefox}/bin/firefox --name firefox "$@"
  '';
in {
  imports = [
  ...
  xdg.desktopEntries = {
    ...
    firefox-existing-profile = {
      name = "Firefox";
      genericName = "Web Browser";
      comment = "Open Firefox with the existing default profile";
      exec = "${firefoxExistingProfile}/bin/firefox-existing-profile %U";
      icon = "firefox";
      terminal = false;
      categories = [
        "Network"
      ...
      settings = {
        ...
        Keywords = "web;browser;mozilla;firefox;";
      };
    };
  };
  ...
  xdg.dataFile."applications/firefox-existing-profile.desktop".text = ''
    [Desktop Entry]
    Type=Application
    Version=1.0
    Name=Firefox
